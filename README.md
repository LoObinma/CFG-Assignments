...
### Vocation 📆: 
I work in childcare as a Nursery Assistant and I'm also a Private Tutor.
...
2. Created a folder on my desktop with the same name and used the command  `git init` to initialise an empty repository in that folder. `ls -a` shows the _.git_ file.

![initialising empty repository using git init](./screenshots/git%20init%202.png)


3. Created a connection between the local and remote repository using the command `git clone + link`.

![creating link between local and remote repository using git clone](./screenshots/git%20clone.png)


4. Used the command `git add` to move changes to _README.md_ file from the working directory ---> staging area, and then `git commit -m` to save the changes in the staging area to the local repository. 

![Using commands git add and git commit](./screenshots/git%20add_git%20commit%202.png)


5. Used the command `git push` to push changes from the local repository ---> remote repository.
...
9. Switched to the local _main_ branch using the command `git switch main`, then used `git fetch` to get the most recent updates from the remote repository. 
The output: `[2c4ba99..2031d43  main            --> origin/main]` shows the remote main branch has new commits that my local main does not.
